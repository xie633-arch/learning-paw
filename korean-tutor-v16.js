...
export function buildTutorPrompt(value = {}) {
  ...
  const focus = settings.focus
    ? settings.focus
    : '我会在开课时告诉你今天看到的具体页码、课文、语法点，或上传教材页面。如果我没有提供，请先只问一个问题确认今天的教材范围，不要凭空断言这一课包含哪些内容。';
...
2. 5–12 分钟｜教材学习：围绕我今天正在看的课文、语法或表达解释重点。教材决定“学什么”，不要另造一套课程顺序。
3. 12–20 分钟｜理解与受控练习：用辨析、替换、造句、课文理解等方式确认我真的懂了。
4. 20–27 分钟｜主动输出：围绕本课做问答、情景对话、复述或中→韩表达，尽可能让我说，而不是你一直讲。
5. 27–30 分钟｜纠错与复盘：总结最值得修正的问题，并确认今天教材目标是否掌握。
...
- 如果我上传教材照片或贴出课文，以我提供的教材页面为准；不要大段复述受版权保护的教材原文。
...
现在开始上课。先根据上面的教材进度给我第一个问题；如果今天的具体教材范围还不够明确，只问一个最必要的确认问题。`;
}
...
curricula.korean = {
  ...
  domain: 'korean',
  status: 'active',
  source: '《延世韩国语》为用户自有教材；Learning Paw 只记录教材进度、调度私教与保存学习事实，不重新分发教材正文。',
  goal: {
...
domainOverrides.korean = {
  status: 'active',
  statusLabel: '可学习',
  description: '以《延世韩国语》为唯一课程主线；Learning Paw 负责教材进度、30 分钟 AI 私教任务与学习记录，墨墨记忆卡负责词汇记忆。',
  modes: [
    { label: '《延世韩国语》教材主线', status: '可用' },
